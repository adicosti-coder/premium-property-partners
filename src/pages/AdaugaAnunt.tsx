...
const AdaugaAnunt = () => {
  ...
  const t = language === "en" ? {
    ...
    pageSubtitle: "Upload photos and let AI generate your listing. After submission, our team will schedule an inspection before publishing.",
    upload: "Add Photos",
    ...
    success: "Listing submitted! Our team will contact you to schedule an inspection.",
    ...
    bathroomsLabel: "Băi",
    priceLabel: "Preț (€)",
    categoryLabel: "Categorie Anunț",
    sale: "Vânzare",
    rent: "Închiriere",
    hotelRegime: "Regim Hotelier",
    ...
    submitBtn: "Trimite pentru Inspecție",
    submitting: "Se trimite...",
    success: "Anunțul a fost trimis! Echipa noastră vă va contacta pentru a programa o inspecție.",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    if (!title.trim()) {
      toast.error(language === "ro" ? "Titlul este obligatoriu" : "Title is required");
      return;
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 pt-24">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-2 text-sm max-w-lg mx-auto">{t.pageSubtitle}</p>
        </div>

        {/* Auth Gate */}
        {!user ? (
          <div className="text-center py-16 space-y-4">
            <LogIn className="w-12 h-12 mx-auto text-muted-foreground/50" />
            <p className="text-muted-foreground">{t.loginRequired}</p>
            <button
              onClick={() => navigate("/auth")}
              className="px-6 py-3 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 transition-all"
            >
              {t.loginBtn}
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <section className="space-y-4">
              {/* Listing Category - 3 options */}
              <div className="space-y-2">
                <Label className="flex items-center gap-2"><FileText className="w-4 h-4" /> {t.categoryLabel}</Label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { value: "vanzare" as const, label: t.sale },
                    { value: "inchiriere" as const, label: t.rent },
                    { value: "regim_hotelier" as const, label: t.hotelRegime },
                  ].map(opt => (
                    <button
                      key={opt.value}
                      type="button"
                      onClick={() => setListingCategory(opt.value)}
                      className={cn(
                        "py-2 px-3 rounded-lg border text-sm font-medium transition-all",
                        listingCategory === opt.value
