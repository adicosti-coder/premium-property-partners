...
import { CheckCircle2, Home, BookOpen, Star, ArrowRight } from "lucide-react";
...
const GOOGLE_REVIEW_URL = "https://g.page/r/CXg7mP2VP_jcEAI/review";
...
const Multumim = () => {
  ...
  const t = {
    title: isRo ? "Mulțumim! Mesajul a fost primit." : "Thank you! Your message has been received.",
    subtitle: isRo
      ? "Un consultant RealTrust te va contacta în maxim 1 oră lucrătoare. Între timp, descoperă mai multe despre noi:"
      : "A RealTrust consultant will reach out within 1 business hour. In the meantime, explore more:",
    ...
    ctaProperties: isRo ? "Vezi apartamentele noastre" : "See our apartments",
    ...
    ctaReview: isRo ? "Lasă-ne o recenzie pe Google" : "Leave us a Google review",
    ctaReviewDesc: isRo ? "Ne ajută enorm — durează 30s" : "It helps us a lot — takes 30s",
        ...
        <div className="container mx-auto px-6 max-w-3xl">
          ...
          <div className="grid gap-4 sm:gap-5">
            ...
            <Link
              ...
              onClick={() => trackConversion({ event: "contact_form_submit", source: "thank_you_cta_guide" })}
              className="group flex items-center gap-4 p-5 sm:p-6 bg-card border-2 border-border hover:border-amber-500 rounded-2xl transition-all hover:shadow-lg"
            >
              <div className="shrink-0 w-12 h-12 rounded-xl bg-amber-500/10 flex items-center justify-center group-hover:bg-amber-500/20 transition-colors">
            ...
            <a
              ...
              onClick={() => trackConversion({ event: "contact_form_submit", source: "thank_you_cta_review" })}
            ...
            >
              <div className="shrink-0 w-12 h-12 rounded-xl bg-blue-950/10 flex items-center justify-center">
                <Star className="w-6 h-6 text-blue-950" fill="currentColor" />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-base sm:text-lg text-blue-950">{t.ctaReview}</h3>
                <p className="text-sm text-blue-950/80">{t.ctaReviewDesc}</p>
              </div>
