...
const Hero = () => {
  ...
  // Parallax via CSS custom property — no React state updates on scroll (main thread safe)
  const bgRef = useRef<HTMLDivElement>(null);
  const contentRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (isMobile) return; // zero parallax on mobile
    ...
    const handleScroll = () => {
      if (!ticking) {
        requestAnimationFrame(() => {
          ...
          // Directly mutate DOM style — avoids React re-render on every scroll frame
          if (bgRef.current) {
            bgRef.current.style.transform = `translateY(${y * 0.35}px) scale(1.05)`;
          }
          if (contentRef.current) {
            contentRef.current.style.opacity = String(Math.max(0, 1 - y * 0.002));
  ...
  // Defer video loading for better LCP — skip on mobile (saves 15-20MB)
  useEffect(() => {
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden pt-28 md:pt-32">
      {/* Background — will-change:transform for GPU compositing, NO filter on scroll */}
      <div
        ref={bgRef}
      ...
      >
        {/* LCP image — always rendered immediately */}
        <img
          src={heroSettings.customFallbackImage || heroImage}
          alt="Apartament de lux Timișoara"
          className={`w-full h-full object-cover ${videoLoaded && !videoError && shouldLoadVideo ? 'opacity-0' : 'opacity-100'} transition-opacity duration-700`}
          width={1920}
          height={1080}
          fetchPriority="high"
          decoding="async"
      ...
      {/* Cinematic vignette — desktop only, static (no animation = no layout shift) */}
      {!isMobile && (
        <div
          className="absolute inset-0 pointer-events-none z-[1]"
          style={{ background: "radial-gradient(ellipse at center, transparent 30%, rgba(0,0,0,0.20) 70%, rgba(0,0,0,0.45) 100%)" }}
        />
      )}

      {/* Film grain — desktop only, use CSS animation (compositor-only, no layout) */}
      {!isMobile && <div className="absolute inset-0 pointer-events-none z-[2] film-grain opacity-[0.04]" />}

      {/* Warm color grading — static, no animation */}
      {!isMobile && (
        <div
          className="absolute inset-0 pointer-events-none z-[1] mix-blend-overlay"
          style={{ background: "linear-gradient(135deg, rgba(212,175,55,0.08) 0%, rgba(180,140,40,0.05) 50%, rgba(139,90,43,0.06) 100%)" }}
        />
      )}

      {/* Lens flare — desktop only, GPU composited (opacity + transform only) */}
      {!isMobile && (
        <>
          <div
            className="absolute top-0 right-0 w-[600px] h-[600px] pointer-events-none z-[1] animate-lens-flare"
            style={{ background: "radial-gradient(ellipse at 80% 20%, rgba(255,215,100,0.15) 0%, rgba(255,200,80,0.08) 25%, transparent 60%)", transform: "translate(20%, -30%)" }}
          />
          <div
            className="absolute top-20 right-40 w-16 h-16 pointer-events-none z-[1] rounded-full animate-flare-orb"
            style={{ background: "radial-gradient(circle, rgba(255,230,150,0.4) 0%, rgba(255,215,100,0.1) 40%, transparent 70%)", filter: "blur(8px)" }}
          />
        </>
      )}
      ...
      {/* Content — direct DOM mutation for scroll, no React re-renders */}
      <div
        ref={contentRef}
        className="container relative z-10 mx-auto px-6 py-20 lg:py-32"
      ...
      {/* Scroll indicator — desktop only, CSS animation (compositor-safe) */}
      {!isMobile && (
        <div
          ...
          onClick={() => {
            const next = document.getElementById('calculator') || document.getElementById('benefits');
            next?.scrollIntoView({ behavior: 'smooth' });
          }}
        >
          <span className="text-sm md:text-base font-bold text-foreground tracking-wide uppercase text-gradient-gold drop-shadow-[0_0_12px_hsl(var(--primary)/0.4)]">
...
// Typing title — 3-line layout
const TypingTitle = ({ title, titleMid, highlight }: { title: string; titleMid: string; highlight: string }) => {
  const { displayedText: titleText, isComplete: titleComplete } = useTypingAnimation({ text: title, speed: 30, delay: 200 });
      ...
      {titleComplete && (
        <span className="block">
          <span className="inline-flex items-baseline gap-1 px-2 py-1 rounded-lg bg-background/35 backdrop-blur-sm border border-border/40">
            ...
            <span className={`inline-block w-0.5 h-[0.9em] bg-primary align-middle transition-opacity duration-300 ${highlightComplete ? "opacity-0" : "animate-pulse"}`} />
          </span>
        </span>
      )}
    </span>
...
// HeroContent component with typing subtitle and CTAs
const HeroContent = ({
  subtitle,
  titleLength,
  highlightLength,
  ...
  return (
    <>
      <p className="text-lg md:text-xl text-foreground max-w-2xl mb-8 leading-relaxed">
        {subtitleText}
        <span className={`inline-block w-0.5 h-[1em] bg-foreground/60 ml-0.5 align-middle transition-opacity duration-300 ${subtitleComplete ? "opacity-0" : "animate-pulse"}`} />
      </p>

      {/* CTAs — use CSS transitions (compositor safe, no layout shift) */}
      <div className={`flex flex-col sm:flex-row gap-3 transition-opacity transition-transform duration-300 ${subtitleComplete ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}>
        <Button
          variant="hero"
          size="xl"
      ...
      {/* Quick Contact Row */}
      <div className={`flex items-center gap-4 mt-4 transition-opacity duration-300 ${subtitleComplete ? 'opacity-100' : 'opacity-0'}`}>
        <a
          href="https://wa.me/40723154520?text=Bună!%20Sunt%20interesat%20de%20serviciile%20RealTrust."
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-4 py-2 bg-[#25D366] hover:bg-[#20BD5A] text-white rounded-lg font-medium text-sm transition-colors shadow-md"
        >
        ...
        <a
          ...
          className="inline-flex items-center gap-2 px-4 py-2 bg-background/80 hover:bg-background/90 text-foreground rounded-lg font-medium text-sm transition-colors border border-border shadow-sm"
        >
