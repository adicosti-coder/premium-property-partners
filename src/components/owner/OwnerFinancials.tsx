...
const OwnerFinancials = ({ propertyId }: OwnerFinancialsProps) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    doc.text("RealTrust - " + t.title, 14, 22);
    doc.setFontSize(10);
    doc.setTextColor(120, 120, 120);
    doc.text(format(new Date(), "MMMM yyyy", { locale: dateLocale }), 14, 30);

    // Summary boxes
    let y = 42;
    doc.setFontSize(12);
    doc.setTextColor(34, 139, 34);
    doc.text(`${t.income}: +${totalIncome.toLocaleString()} €`, 14, y);
    doc.setTextColor(220, 20, 60);
    doc.text(`${t.expenses}: -${totalExpenses.toLocaleString()} €`, 14, y + 8);
    doc.setTextColor(40, 40, 40);
    doc.text(`${t.netProfit}: ${netProfit >= 0 ? "+" : ""}${netProfit.toLocaleString()} €`, 14, y + 16);
    ...
    doc.text(t.category, 55, y);
    doc.text("Type", 110, y);
    doc.text(t.amount, 145, y);
    ...
    financials?.forEach(record => {
      ...
      doc.text(format(parseISO(record.date), "dd/MM/yyyy"), 14, y);
      doc.text(getCategoryLabel(record.category), 55, y);
      doc.text(record.type, 110, y);
      const prefix = record.type === "income" ? "+" : "-";
      doc.text(`${prefix}${Number(record.amount).toLocaleString()} €`, 145, y);
      y += 7;
    });
    ...
    doc.text("Generated by RealTrust © " + new Date().getFullYear(), 14, 285);

    doc.save(`raport-financiar-${format(new Date(), "yyyy-MM")}.pdf`);
  };
