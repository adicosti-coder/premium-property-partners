-- Raport pe cuvânt cheie: câte anunțuri aduce fiecare, pe platformă
...
AS $$
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE p.created_at >= now() - make_interval(days => GREATEST(p_days, 1))
      ...
      AND k.keyword = ANY (p.search_keywords)
...
REVOKE ALL ON FUNCTION public.get_keyword_scan_report(integer, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_keyword_scan_report(integer, text) TO authenticated, service_role;