...
Deno.serve(async (req) => {
  ...
  if (!pending || pending.length === 0) {
    return new Response(JSON.stringify({ classified: 0, message: "no_pending" }), {
  ...
  try {
    ...
    for (const r of results) {
      ...
      // append tag without clobbering existing tags
      if (tag) {
  ...
  return new Response(JSON.stringify({ classified, approvals_created: approvalsCreated, batch_size: pending.length }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
});
