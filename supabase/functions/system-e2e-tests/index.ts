...
// Lightweight SEO smoke test — verifies the site is reachable and returns
// well-formed HTML (title + meta description). Avoids the heavy Gemini audit
// in seo-ai-optimizer which exceeds the 150s edge function IDLE_TIMEOUT.
// The full AI audit runs separately via the seo.ai_optimizer_audit cron job.
async function runSeo(_SUPABASE_URL: string, _SERVICE_KEY: string, seoUrl: string) {
  const start = Date.now();
  let passed = false; let details: any = {}; let err: string | null = null;
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), 20_000); // 20s hard cap
  try {
    const r = await fetch(seoUrl, {
      method: "GET",
      headers: { "User-Agent": "RealTrust-E2E-Smoke/1.0", Accept: "text/html" },
      signal: ctrl.signal,
      redirect: "follow",
    });
    ...
    passed = r.ok && Boolean(titleMatch) && hasMetaDesc && hasH1 && sizeOk;
    if (!passed) {
      err = `HTTP ${r.status}; title=${Boolean(titleMatch)}, meta=${hasMetaDesc}, h1=${hasH1}, size=${html.length}`;
    }
  } catch (e: any) {
    err = `${e?.name || "Error"}: ${e?.message || String(e)}`.slice(0, 1000);
  } finally {
